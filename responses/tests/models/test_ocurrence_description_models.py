...
class OcurrenceDescriptionModelTest(SetUpInitial):
    def setUp(self):
        ...
        self.ocurrence = OcurrenceDescription.objects.create(
            name="Não se aplica", owner_id=self.user.id
        )
    ...
    def test_create_meta(self):
        ...
        self.assertIsNotNone(
            self.ocurrence.created_at
        )  # Verifica se o campo created_at é preenchido

    def test_create_null(self):
        """Testa a criação de um objeto OcurrenceDescription com o campo 'name' em branco."""
        with self.assertRaises(IntegrityError):
            OcurrenceDescription.objects.create(
                name=None, owner_id=self.user.id
            )
    ...
    def test_meta_ordering(self):
        ...
        ocurrence1 = OcurrenceDescription.objects.create(
            name="Falha na assistência", owner_id=self.user.id
        )
        ocurrence2 = OcurrenceDescription.objects.create(
            name="Falha do transporte", owner_id=self.user.id
        )
        ocurrences = OcurrenceDescription.objects.all()
        self.assertEqual(
            list(ocurrences), [self.ocurrence, ocurrence1, ocurrence2]
        )
